...
class ExcelLoader:
    # Expected column names from DSLD SQL exports
    EXPECTED_COLUMNS = [
        'Region', 'Subdivision1', 'Lot1', 'StreetAddress', 'City', 'State',
        'ZipCode', 'Plan1', 'Living', 'PermitNo1', 'ConstCompleteDate1',
        'Super', 'Tech', 'RTIN', 'PDWCreated1', 'PDWFails1',
        'FinalCreatedDate', 'FinalizationDate', 'PassFail1',
        'ConstCompleteDate', 'TargetClosingDate', 'ActualClosingDate',
        'ElecOption', 'SupplierName', 'Tonnage', 'RefrigeratorModel',
        'RangeModel', 'TDLCFM', 'LTOCFM', 'BDCFM', 'MVCFM',
        'ReturnIWC', 'SupplyIWC', 'BlowerCFM', 'MeasuredCFM', 'FWD',
        'MeasuredWattage', 'Charge'
    ]
    
    @staticmethod
    def _normalize_columns(df):
        """Normalize column names: strip whitespace, fix common variations"""
        # Build lookup: lowercase/stripped -> expected name
        expected_map = {}
        for col in ExcelLoader.EXPECTED_COLUMNS:
            expected_map[col.lower().strip()] = col
            expected_map[col.lower().replace(' ', '').strip()] = col
        
        rename_map = {}
        for col in df.columns:
            clean = str(col).strip()
            lookup = clean.lower().replace(' ', '')
            if lookup in expected_map:
                target = expected_map[lookup]
                if clean != target:
